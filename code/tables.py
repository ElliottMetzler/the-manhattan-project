...
def price_and_abv_preprocessed():
    ...
    combined = (
        recoded.merge(ingredient_prices, how="left", on="ingredient")
        .merge(abv_data, how="left", on="ingredient")
        .fillna(0)
        .assign(ounces_alcohol=lambda df_: df_["amount"] * df_["abv_percent"])
        .groupby("strdrink")
        .agg(
            {
                "ingredient": "count",
...
def summary_of_usage():
    ...
    data = dum_df.describe().transpose().sort_values("mean", ascending=False).head(10)
    data = data[["mean"]]
...
def get_amount_table():
    ...
    measure_long = qp.shape_data_long(
        df, measure_cols, "strmeasure", "_clean", "amount"
    )
...
def drop_big_drinks():
    """Filter out drinks that are either small enough to be shots or big enough to be too large and skew analysis"""
    df = get_amount_table()
...
def most_popular_liquor_table():
    """Create Most Popular Liquor Table"""

    df = drop_all_zero_dummies()
    df = df.describe().transpose().sort_values("mean", ascending=False).head(15)
    df = df[["mean"]]
...
def heat_price_corr_heat():
    ...
    corr_heat = sns.heatmap(
        corr, xticklabels=corr.columns, yticklabels=corr.columns, cmap="RdBu"
    )
