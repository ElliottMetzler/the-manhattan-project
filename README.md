...
Additionally, our search algorithm carries some limitations of varying significance. For one, as we performed our final applet testing, we noticed that there were instances where searching for drinks containing "gin" returned drinks containing both "gin" and "ginger." Due to time constraints we were unable to adjust and implement a solution. Though we are currently unaware of any other issues with our base searching mechanism, it is possible other minor search issues exist. A larger limitation with our search algorithm is in its inability to categorize or rank drinks. We do not have any sort of popularity metric for our drinks, so the output based on the parameters is random so long as enough cocktails fit the search criteria. A nice extension in the future may be to find data on drink popularity and implement this into our search algorithm. Another good extension requiring more robust input data would be to allow for more granular search and price estimates based on a more thorough ingredient price search. We could indeed price out Grey Goose Martinis as opposed to Kirkland brand vodka Martinis with additional data and time to implement adjustments.
